...
public class FakePlayers implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (sender.hasPermission("fakeplayers.cmd")) {
            if (args.length == 0) {
                ...
                switch (args[0]) {
                    ...
                    case "list":
                        ...
                        break;
                    case "reload":
                        FakePlugin.getPlugin().reload();
                        sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.reload", args));
                        break;
                    case "macro":
                        macro(sender, args);
                        break;
                            ...
                            if (args.length >= 3) {
                                if (!args[1].equalsIgnoreCase("All")) {
                                    ...
                                    if (player != null) {
                                        ...
                                        sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.action.invalid-player", args));
                                    }
                                } else {
                                    for (FakePlayer player : FakePlayer.getFakePlayers().values()) {
                                        action(sender, player, args);
                                    }
                                }
            ...
            sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.no-permissions", args));
        }
        return true;
    }
    ...
    private void summon(CommandSender sender, String name, int number, String[] args) {
        if (number == 1) {
            if (FakePlayer.create(name)) {
                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.summon.success-one", args));
            } else {
                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.summon.failed", args));
            }
        } else if (number < 1) {
            sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.summon.incorrect-number", args));
        } else {
            int addition = 0;
            ...
            for (int i = addition; i < number + addition; i++) {
                final int I = i;
                Bukkit.getScheduler().scheduleSyncDelayedTask(FakePlugin.getPlugin(), () -> FakePlayer.create(name + (I + 1)), I * FakePlugin.getPlugin().config.getInt("tick-delay-between-joins"));
            }
            sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.summon.trying-amount", args));
        }
    }
        ...
        if (name.equalsIgnoreCase("All")) {
            ...
            if (player != null) {
                if (FakePlayer.getFakePlayer(name) != null) {
                    ...
                    sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.chat.not-a-fake-player", args));
                }
            } else {
                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.chat.player-is-not-online", args));
            }
        }
    }
    ...
    private void disband(CommandSender sender, String name, String[] args) {
        ...
        if (name.equalsIgnoreCase("All")) {
            FakePlayer.getFakePlayers().values().forEach(player -> {
                player.removePlayer();
                player.destroy();
            });
            sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.disband.all-disbanded-success", args));
        } else {
            ...
            if (fakePlayer != null) {
                ...
                fakePlayer.destroy();
                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.disband.one-disbanded-success", args));
            } else {
                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.disband.failed", args));
            }
        }
    }
    ...
    private void macro(CommandSender sender, String[] args) {
        if (args.length >= 3) {
            ...
            if (fakePlayer != null) {
                if(args.length < 4) {
                    sender.sendMessage("Please specify a name for the macro! ");
                    return;
                }
                String macro = args[3];
                if (args.length == 4) {
                    switch (args[1]) {
                        case "save":
                            try {
                                ...
                                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.macro.macro-successful-save", args));
                            } catch (IOException e) {
                                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.macro.macro-failed-save", args));
                        ...
                        case "load":
                            try {
                                ...
                                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.macro.macro-successful-load", args));
                            } catch (IOException e) {
                                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.macro.macro-failed-load", args));
                                Bukkit.getLogger().warning("Could not load macro: '" + macro + "'.");
                            }
                            break;
                        case "perform":
                            List<Action> actionsCopy = new ArrayList<>(fakePlayer.actions);
                            Bukkit.getScheduler().runTask(FakePlugin.getPlugin(), () -> {
                                try {
                                    sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.macro.macro-successful-load", args));
                                    sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.macro.macro-perform", args));
                                    fakePlayer.actions = Macro.loadMacro(macro).getActions();
                    ...
                    if (args[1].equals("perform")) {
                        try {
                            ...
                            try {
                                ...
                                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.macro.incorrect-number", args));
                            }
                            fakePlayer.actions = actionsCopy;

                            sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.macro.macro-successful-load", args));
                            sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.macro.macro-macro-perform", args));
                        } catch (IOException e) {
                            sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.macro.macro-failed-load", args));
                            Bukkit.getLogger().warning("Could not load macro: '" + macro + "'.");
                        }
                ...
                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.macro.invalid-player", args));
            }
        } else {
            Bukkit.dispatchCommand(sender, "fakeplayers");
        }
    ...
    private void action(CommandSender sender, FakePlayer player, String[] args) {
        switch (args[2].toLowerCase()) {
            ...
            case "chat":
                ...
                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.action.action-success", args));
                break;
            case "teleport":
                if (args.length == 4) {
            ...
            case "clear":
                ...
                sender.sendMessage(FakePlugin.getConfigMessage(FakePlugin.getPlugin().config, "messages.action.actions-clear", args));
                break;
        }
    }
}
